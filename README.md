...
We can check that, on training DCGAN on LSUN-Bedroom, the measured critical batch size when the stopping condition is FID ≦ 50 is larger than the measured critical batch size when the stopping condition is FID ≦ 70. Please see the graph below.
...
The graph on the left is a modified version of Figure 2, and the graph on the right is a modified version of Figure 4.

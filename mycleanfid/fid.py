...
import zipfile
import pickle
from mycleanfid.utils import *
from mycleanfid.features import build_feature_extractor, get_reference_statistics
from mycleanfid.resize import *
...
"""
def compare_folders(fdir1, fdir2, first, feat_model, mode, num_workers=12,
                    batch_size=64, device=torch.device("cuda"), verbose=True,
                    custom_image_tranform=None):
    ...
    if first:
        ...
        with open('mu1.dump','wb') as f:
            pickle.dump(mu1, f)
        with open('sigma1.dump','wb') as g:
            pickle.dump(sigma1, g)
    else:
        with open('mu1.dump','rb') as f:
            mu1 = pickle.load(f)
        with open('sigma1.dump','rb') as g:
            sigma1 = pickle.load(g)
...
"""
def compute_fid(fdir1=None, fdir2=None, first=True, gen=None,
            mode="clean", num_workers=12, batch_size=32,
            device=torch.device("cuda"), dataset_name="FFHQ",
            dataset_res=1024, dataset_split="train", num_gen=50_000, z_dim=512,
    ...
    if fdir1 is not None and fdir2 is not None:
        ...
        score = compare_folders(fdir1, fdir2, first, feat_model,

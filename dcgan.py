...
if __name__ == '__main__':
    ...
    ##Adam##
    optimizerD = optim.Adam(netD.parameters(), lr=args.d_lr, betas=(args.beta1, args.beta2))
    optimizerG = optim.Adam(netG.parameters(), lr=args.g_lr, betas=(args.beta1, args.beta2))

    ##AdaBelief##
    #optimizerD = AdaBelief(netD.parameters(), lr=args.d_lr, eps=1e-16, betas=(args.beta1, args.beta2), rectify=True,weight_decay=0, amsgrad=False, weight_decouple=True, fixed_decay=False)
    #optimizerG = AdaBelief(netG.parameters(), lr=args.g_lr, eps=1e-16, betas=(args.beta1, args.beta2), rectify=True, weight_decay=0, amsgrad=False, weight_decouple=True, fixed_decay=False)

    ##RMSProp##
    #optimizerD = optim.RMSprop(netD.parameters(), lr=args.d_lr, alpha=0.99, eps=1e-08)
    #optimizerG = optim.RMSprop(netG.parameters(), lr=args.g_lr, alpha=0.99, eps=1e-08)
    ...
    for epoch in range(args.num_epochs):
        for i, data in enumerate(dataloader, 0):
            ...
            if (iters > 10000) & (iters % 500 == 0):
                print('iters:%d, calculateFID...' % iters)
                fid_batch_size = 64
                fake_image_num_sample = 50000
                generation_loop_iter = int(fake_image_num_sample/fid_batch_size)
                netG.eval()
